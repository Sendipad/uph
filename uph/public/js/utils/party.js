...
uph.party = {
        ...
        if (r.message) {
          ...
          uphdialog.set_primary_action(__("Set Party"), () => {
            ...
            if (values.is_default_for_party_master) {
              frappe.call({
                method:
                  "uph.party.controllers.party.set_party_as_default_for_party_master",
                args: {
                  party: values.party,
    ...
    frappe.db.get_value(
      ...
      (r) => {
        if (r && r.message) {
          ...
          frm.set_value(
            fn.party_fieldname,
            r.message[fn.default_role_fieldname]
          );
  ...
  get_dialog_field: function (frm, pm_details, fn) {
    ...
    const currencyLabel = parties.some((p) => p.currency)
      ? __(" (Currency)")
      : "";
    ...
    const fields = [
      ...
      },
      {
        ...
        label: __("Select Party"),
        options: parties.map(
          (p) => `${p.name}${p.currency ? ` (${p.currency})` : ""}`
        ),
      },
    ];
          ...
          frappe.call({
            ...
            callback: function (r) {
              if (r.message) {
                ...
                party_master_doc[fn.default_role_fieldname] =
                  selectedParty.name;
  ...
  get_party_master_details: function (frm, fn, callback) {
    ...
    frappe.call({
      method:
        "uph.party.controllers.party.get_party_master_details_with_parties",
      args: args,
      callback: (r) => {
        if (r.exc) {
  ...
  handle_party_master_change: function (frm, fieldname) {
    ...
    this.show_party_selection_dialog_callback(
      frm,
      frm.doc.party_master,
      false,
      (values) => {
  ...
  setup_queries_on_child: function (frm, child_doctype, fieldname) {
    ...
    if (child_fieldname) {
      ...
      frm.fields_dict[child_fieldname].grid.get_field(
        "party_master"
      ...
      frm.fields_dict[child_fieldname].grid.get_field(fieldname).get_query =
            ...
            fields: [
              {
                ...
                onchange: function () {
                  ...
                  let party_data = parties.find(
                    (p) => p.name === selected_party
                  );
  ...
  check_duplicate_voucher_for_party_master: function (
    ...
    frappe.call({
      method:
        "uph.party.controllers.party.check_duplicate_voucher_party_master",
      args: args,
      callback: function (r) {
            ...
            if (triggered_before_submit) {
              ...
              frappe.confirm({
                ...
                primary_action: () => {
                  frappe.call({
                    method:
                      "uph.party.controllers.party.allow_duplicate_submission",
                    args: {
                      doctype: frm.doctype,
