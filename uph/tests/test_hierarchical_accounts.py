...
class TestHierarchicalAccounts(FrappeTestCase):
    def setUp(self):
        ...
        # Setup Accounts
        self.root_acc = self.create_account(
            "Test Acc Root Account", "Receivable", is_group=1, currency=""
        )
    ...
    def test_group_account_expansion(self):
        self.root_pm.reload()
        self.root_pm.append(
            "accounts", {"company": self.company, "account": self.root_acc.name}
        )
        self.root_pm.save(ignore_permissions=True)

        # Create a leaf under Root Account for USD
        usd_leaf = self.create_account(
            "Test Root USD Leaf", "Receivable", currency=self.currency_usd
        )
