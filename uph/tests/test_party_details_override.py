...
class TestPartyDetailsOverride(FrappeTestCase):
	def setUp(self):
		...
		self.address.append("links", {"link_doctype": "Party Master", "link_name": self.child_pm.name})
		...
		self.contact.append("links", {"link_doctype": "Party Master", "link_name": self.child_pm.name})
	...
	def test_pm_details_and_contacts(self):
		details = uph_get_party_details(party=self.customer.name, party_type="Customer", company=self.company)
	...
	def test_non_destructive_enrichment(self):
		...
		parent_pm.append("accounts", {"company": self.company, "account": pm_group.name})
		parent_pm.save(ignore_permissions=True)
