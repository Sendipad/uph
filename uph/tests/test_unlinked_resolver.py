...
class TestUnlinkedResolver(FrappeTestCase, AccountsTestMixin):
	def setUp(self):
		...
		frappe.db.delete("Customer", {"customer_name": ["like", "_Test Unlinked Customer %"]})
	...
	def test_link_customer_keeps_customer_name_and_updates_vouchers(self):
		...
		pt_row = next((d for d in settings.party_types if d.party_type == "Customer"), None)
		added_pt = False
		if not pt_row:
			pt_row = settings.append("party_types", {"party_type": "Customer", "reqd": 0, "allowed": 0})
			added_pt = True
		...
		try:
			...
			original_customer_name = frappe.db.get_value("Sales Invoice", si.name, "customer_name")
			...
			updated_customer_name = frappe.db.get_value("Sales Invoice", si.name, "customer_name")
			self.assertEqual(
			...
			if added_pt:
				settings.party_types = [d for d in settings.party_types if d.party_type != "Customer"]
			else:
	...
	def test_link_customer_updates_large_volume_vouchers(self):
		...
		jea_row = next(
			(d for d in settings.document_types if d.document_type == "Journal Entry Account"),
			None,
		)
		added_jea = False
		if not jea_row:
		...
		try:
			...
			pe_linked = (
				...
				.select(Count("*").as_("cnt"))
				.where(
					(pe.party == customer.name) & (pe.party_type == "Customer") & (pe.party_master == pm.name)
				)
				.run(as_dict=True)[0]["cnt"]
			)
			self.assertEqual(
