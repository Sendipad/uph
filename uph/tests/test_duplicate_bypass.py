...
class TestDuplicateBypass(FrappeTestCase, AccountsTestMixin):
	def setUp(self):
		...
		# Cleanup
		frappe.db.delete("Sales Invoice", {"company": self.company, "customer": self.customer})
	...
	def test_duplicate_check_disabled(self):
		...
		res = check_duplicate_voucher_party_master(self.pm.name, "Sales Invoice", self.si.posting_date)
