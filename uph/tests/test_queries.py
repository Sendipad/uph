...
class TestQueries(FrappeTestCase, AccountsTestMixin):
	...
	def test_get_party_master_parties_db_with_roles(self):
		...
		if pm:
			...
			result = get_party_master_parties_db(pm[0], all_roles=False, roles=["Customer"])
			self.assertIsInstance(result, list)
			# Test with non-matching role
			result_empty = get_party_master_parties_db(pm[0], all_roles=False, roles=["Supplier"])
	...
	def test_get_leaf_party_master_list(self):
		...
		result = get_leaf_party_master_list_from_any_node({"party_master": [group_pm.name]})
		self.assertIn(leaf_pm.name, result)

	def test_party_master_link_query(self):
		# Create a party master
		pm_name = "_Test PM Link Query"
		parent = frappe.db.get_value("Party Master", self.party_master, "parent_party_master")
		frappe.get_doc(
			{
				"doctype": "Party Master",
				"party_name": pm_name,
		...
		self.assertIn(frappe.db.get_value("Party Master", {"party_name": pm_name}, "name"), names)
...
def create_customer(name, party_master=None):
	...
	cust = frappe.get_doc({"doctype": "Customer", "customer_name": name, "party_master": party_master})
	cust.name = _unique_docname("Customer", "TEST-CUST")
	cust.insert(ignore_permissions=True)
	return cust.name
