...
def execute():
	...
	for party_type in party_type_list:
		...
		for dt_conf in settings_doc.document_types:
			...
			for party_name, party_master in party_pm_map.items():
				...
				# Count first
				count_result = (
					frappe.qb.from_(tbl).select(Count("*").as_("cnt")).where(conditions).run(as_dict=True)
				)
				count = count_result[0]["cnt"] if count_result else 0

				if count > 0:
					frappe.qb.update(tbl).set(tbl.party_master, party_master).where(conditions).run()
	...
	if total_updated:
		frappe.logger("uph").info(f"Patch fix_voucher_pm: Total {total_updated} voucher records updated")
		frappe.msgprint(
