...
class PartyMasterSettings(Document):
	...
	def validate_party_master_fields_options(self):
		...
		for df, option in new_row_mapped.items():
			...
			for party_name in party_types:
				...
				party_field = next((field for name, field in option_list if name == party_name), None)
			...
			new_row_mapped[df] = '\n'.join(f"{name}:{field}" for name, field in option_list)
	...
	def sync_update_to_doctype_fields(self,force_update=False):
		...
		doc_field='Party Master Settings DocType'
		for d in self.document_types:
			meta=frappe.get_meta(d.document_type)
			if d.get('party_master_custom_field'):
				continue
			cf=frappe.get_doc('Custom Field',{'dt':d,'fieldtype':'Link','options':'Party Master'})
...
def create_custom_party_master_field(docfield, update=False, field_properity=None):
    ...
    # Build fetch logic
    fetch_from = f"{docfield.get('party_fieldname')}.party_master" if not docfield.get('party_type') else ""
...
def get_default_doctypes(doctype=None,field_properity=None):
    document_type = {
        'Sales Invoice': {'document_categories': 'Selling DocType', 
                          'party_fieldname': 'customer', 'party_type': 'Customer','field_properity': {'insert_after':'company'}},
        'Delivery Note': {'document_categories': 'Selling DocType', 'party_fieldname': 'customer', 'party_type': 'Customer'},
        'Sales Order': {'document_categories': 'Selling DocType', 'party_fieldname': 'customer', 'party_type': 'Customer'},
        'Purchase Invoice': {'document_categories': 'Purchasing DocType', 'party_fieldname': 'supplier', 'party_type': 'Supplier'},
        'Purchase Receipt': {'document_categories': 'Purchasing DocType', 'party_fieldname': 'supplier', 'party_type': 'Supplier'},
        'Purchase Order': {'document_categories': 'Purchasing DocType', 'party_fieldname': 'supplier', 'party_type': 'Supplier'},
        'Payment Entry': {'document_categories': 'Dynamic type As Parent', 'party_fieldname': 'party', 'party_type_fieldname': 'party_type', 'is_dynamic_party_type': 1},
        'Journal Entry Account': {'parent_doctype': 'Journal Entry', 'document_categories': 'Dynamic Type As Child', 'party_fieldname': 'party', 'party_type_fieldname': 'party_type', 'is_dynamic_party_type': 1},
        'Expense Claim': {'document_categories': 'Employee DocType', 'party_fieldname': 'employee', 'party_type': 'Employee'},
		'Payment Reconciliation': {'reqd':0,'document_categories': 'Dynamic type As Parent', 'party_fieldname': 'party', 'party_type_fieldname': 'party_type', 'is_dynamic_party_type': 1,'enabled':0},
    	}
    if not doctype:
...
def setup_initial_document_types():
    ...
    for doc, options in docs.items():
        if not frappe.db.exists('DocType',doc):
            continue
        if frappe.db.exists('Party Master Settings DocType', {'parent':'Party Master Settings','document_type': doc}) and not update_exist:
            continue
        document_type = {k: v for k, v in docs.get(doc).items() if k != 'field_properity'}
...
def setup_party_master_mapping_fields():
    ...
    fields=[df.fieldname for df in fields if df.fieldname not in ('default_currency','default_customer','default_supplier','is_group','lft','rgt','old_parent','party_type','accounts')  and df.fieldtype not in ('Section Break','Column Break','Tab Break')]
    settings = frappe.get_doc('Party Master Settings')
    for f in fields:
    	if f in common_maps:
        	settings.append('party_master_fields',{'fieldname':f,'options':common_maps.get(f)})
        	continue
...
def setup_party_types_table(reset=False):
    ...
    for d in exist_party_type:
        if d not in party_types:
            settings.party_types = [x for x in settings.party_types if x.get('party_type') != d]
    settings.save()
def party_types_dict(party_type):
    return frappe._dict({
			...
			'rule_fieldname':'default_currency' if party_type in ('Customer','Supplier') else '',
		})
...
def get_party_type_party_master_rules_dict():
    ...
    for d in doc.get('party_types'):
        result.update({d.get('party_type'):{'reqd':d.get('reqd'),'allowed':d.get('allowed'),'rule_fieldname':d.get('rule_fieldname')}})
