...
class PartyMaster(NestedSet):
	...
	if TYPE_CHECKING:
		...
		default_party_analytic_accounting: DF.Link | None
		default_price_list: DF.Link | None
		default_project: DF.Link | None
		disabled: DF.Check
	...
	def _validate_accounts_uniqueness(self) -> None:
		...
		for row in self.accounts:
			...
			if key in seen:
				frappe.throw(
					_("Duplicate account configuration for Company {0} and Currency {1}").format(
						row.company, row.currency or _("Any")
					)
				)
			seen.add(key)
	...
	def _validate_parent_numbering(self) -> None:
		...
		enforce = frappe.db.get_single_value("Party Master Settings", "enforce_parent_numbering")
		if not enforce:
			return

		parent_number = frappe.db.get_value("Party Master", self.parent_party_master, "party_number")
		if parent_number and not self.party_number.startswith(parent_number):
			frappe.throw(
				_("Party Number {0} must start with parent Party Number {1}").format(
					self.party_number, parent_number
				)
			)
	...
	def update_linked_parties_details(self):
		...
		for p in self.get("parties"):
			...
			if changed:
				party_doc.save()
				frappe.msgprint(
					_("{0} Named {1}'s details have been updated").format(_(self.party_type), p.party),
					alert=1,
				)
	...
	def set_party_master(self, selection=None, **kwargs):
		...
		for p in selection:
			...
			if party.get("party_master") is None or party.get("party_master") == "":
				...
				self.add_comment(
					"Comment",
					_("{0} has linked {1} {2}").format(frappe.session.user, _(party.doctype), party.name),
				)
		self.reload()
		self.save()
	...
	def _build_parties_query(self, filters):
		...
		for p in pt:
			...
			if unlinked:
				conditions = [
					Case().when(getattr(doctype, party_name_field).like(f"%{word}%"), 1).else_(0)
					for word in words
				]
...
def get_party_master_balances(company, name=None):
	...
	for pt, p_names in pt_to_parties.items():
		...
		pm_map = frappe.db.get_all(pt, filters={"name": ["in", p_names]}, fields=["name", "party_master"])
		for m in pm_map:
			if m.party_master:
	...
	if name:
		...
		map_query = """
            ...
            WHERE req.name IN %(req_names)s
              ...
              AND req.lft > 0
        """
		mappings = frappe.db.sql(map_query, {"req_names": name, "leaf_names": pm_names_in_gl}, as_dict=True)
		for m in mappings:
		...
		for req_pm in target_req_pms:
			...
			for existing in party_balances[req_pm]:
				if existing["currency"] == entry.currency:
					...
					break
			if not found:
				party_balances[req_pm].append({"currency": entry.currency, "amount": entry.balance})
...
def get_next_party_master_number(parent=None, is_group=0):
	...
	try:
		...
		frappe.log_error("Party Number Generation Error", f"{e}\n{traceback.format_exc()}")
		raise
...
def get_totals_number_unlinked_parties(filters=None):
	...
	for p in ptype:
		if frappe.get_meta(p.get("name")).has_field("party_master"):
			result.update(
				{p.get("name"): frappe.db.count(p.get("name"), filters={"party_master": ["is", "not set"]})}
			)
...
def assign_party_master_for_selections_list(assign_parties):
	for p in assign_parties:
		party_type, party = p.get("party_type"), p.get("party")  # Extract values from list
...
def assign_party_master_for_selection(old_party_master, new_party_master, selections):
	for p in selections:
		if not isinstance(p, list) or len(p) != 2:
			frappe.throw(_("Invalid data format for selection. Expected list of lists."))
...
def _check_permission(doctype, name, permission="read"):
	...
	if not frappe.has_permission(doctype, permission, doc=name):
		frappe.throw(
			_("You don't have permission to {0} {1} {2}").format(permission, _(doctype), name),
			frappe.PermissionError,
		)
...
def get_parties(party_master, fromdb=False, party_type=None):
	...
	for p in party_type:
		...
		if pnf:
			fields.extend([getattr(ptDocType, pnf), getattr(ptDocType, pnf).as_("party_name")])
		if cf:
			fields.extend([getattr(ptDocType, cf), getattr(ptDocType, cf).as_("currency")])

		query = frappe.qb.from_(ptDocType).select(*fields).where(ptDocType.party_master == pm.name)
