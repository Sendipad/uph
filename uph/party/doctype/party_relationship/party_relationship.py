...
class PartyRelationship(Document):
	...
	def _validate_circular_dependency(self):
		...
		"""
		rel_type = frappe.get_cached_doc("Party Relationship Type", self.relationship_type)
		...
		while queue:
			...
			if current == self.subject_party:
				frappe.throw(
					_("Circular dependency detected: {0} is already related to {1} via {2}.").format(
						frappe.bold(self.subject_party),
						frappe.bold(self.object_party),
						frappe.bold(self.relationship_type),
					),
			...
			new_doc.update(
				{
					...
					"notes": _("Auto-created reciprocal relationship from {0}").format(self.name),
					"is_guarantor": self.is_guarantor if rel_type.is_symmetric else 0,
					"consolidate_financials": (self.consolidate_financials if rel_type.is_symmetric else 0),
				}
			)
	...
	def _delete_reverse_relationship(self):
		...
		if reverse_name:
			try:
				...
				frappe.delete_doc("Party Relationship", reverse_name, ignore_permissions=True)
			finally:
...
@frappe.whitelist()
def get_related_parties(party_master: str, relationship_type: str | None = None):
	"""
