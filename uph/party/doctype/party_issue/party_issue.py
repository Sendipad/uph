...
def on_doctype_update():
	...
	frappe.db.add_index("Party Issue", ["party_master", "status", "issue_type"], "idx_party_status_type")

	# 2. Performance index for reference-based lookup
	frappe.db.add_index("Party Issue", ["reference_doctype", "reference_name"], "idx_ref_doc")

	# 3. Dashboard/Priority index
	frappe.db.add_index("Party Issue", ["status", "severity_rank", "detected_on"], "idx_dashboard_order")
