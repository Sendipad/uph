...
frappe.ui.form.on("Party Analytic Accounting", {
	refresh(frm) {
		...
		if (frm.doc.party_master) {
			frm.fields_dict["parties"].grid.add_custom_button(__("Fetch Related Parties"), () =>
...
function hide_parties_add_button(frm) {
	frm.get_field("parties").grid.wrapper.find(".grid-add-row").hide();
}
	...
	frappe.call({
		...
		callback(r) {
			...
			let filtered = parties.filter((p) => {
				return !existing.some((e) => e.party_type === p.party_type && e.party === p.party);
			});
