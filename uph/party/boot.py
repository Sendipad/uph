...
from uph.party.controllers.cache_utils import get_party_master_depends_on_fields, get_pm_doctypes
...
def add_pm_doctypes(bootinfo):
	...
	try:
		...
		is_setup_admin = frappe.session.user == "Administrator" or "System Manager" in frappe.get_roles(
			frappe.session.user
		)
