...
class PartyMergeService:
	...
	def merge(
		...
		if not frappe.has_permission("Party Master", "write"):
			frappe.throw(_("Insufficient permissions to merge parties"), frappe.PermissionError)
		...
		if not frappe.db.exists("Party Master", primary_pm):
			frappe.throw(_("Primary Party Master {0} does not exist").format(primary_pm))
		if not frappe.db.exists("Party Master", secondary_pm):
			frappe.throw(_("Secondary Party Master {0} does not exist").format(secondary_pm))
		...
		try:
			...
			if not frappe.cache.set(lock_key, frappe.session.user, nx=True, ex=120):
				frappe.throw(
					_("A merge operation is already in progress for these parties. Please try again later.")
				)
			...
			try:
				...
				return {
					"success": True,
					"message": _("{0} has been merged into {1}").format(secondary_pm, primary_pm),
				...
				frappe.log_error(
					...
					message=f"Failed to merge {secondary_pm} into {primary_pm}: {e!s}\n\nLog: {self.merge_log}",
				)
				raise
			finally:
	...
	def _analyze_linked_parties(self, pm_name: str) -> dict:
		...
		for p in parties:
			...
			if rule_fieldname:
				party_info["rule_value"] = frappe.db.get_value(party_type, party_info["name"], rule_fieldname)

			result[party_type].append(party_info)

		return result

	def _classify_merge_types(self, primary_parties: dict, secondary_parties: dict) -> dict:
		"""
	...
	def _execute_party_merge(self, primary_party: str, secondary_party: str, party_type: str):
		"""
		...
		try:
			...
			self._log(f"Successfully merged {party_type} '{secondary_party}' into '{primary_party}'")
		except Exception as e:
	...
	def _manual_party_merge(self, primary_party: str, secondary_party: str, party_type: str):
		"""
		...
		frappe.delete_doc(party_type, secondary_party, force=True, ignore_permissions=True)
	...
	def _execute_party_relink(self, party_name: str, party_type: str, new_pm: str):
		...
		self._log(f"Re-linked {party_type} '{party_name}' from '{old_pm}' to '{new_pm}'")
		...
		for parent_doctype in ["Address", "Contact"]:
			...
			for link in links:
				...
				if not existing:
					...
					self._log(f"Transferred {parent_doctype} '{link.parent}' link to '{new_party}'")
	...
	def _transfer_pm_dynamic_links(self, old_party: str, new_party: str, party_type: str):
	...
	def _transfer_pm_child_tables(
		self, primary_pm: str, secondary_pm: str, fields_to_keep: dict | None = None
	):
		"""
	...
	def _delete_secondary_pm(self, secondary_pm: str, primary_pm: str):
		...
		primary_doc.add_comment(
			"Info",
			_("Merged Party Master {0} into this record. Merge completed by {1} at {2}").format(
				secondary_pm, frappe.session.user, now_datetime()
			),
		)
