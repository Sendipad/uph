...
@frappe.whitelist()
def get_party_health_detail(party_master: str, reference_doctype: str | None = None):
	"""
		...
		vouchers.append(
			{
				...
				"docstatus": (
					frappe.db.get_value(issue.reference_doctype, issue.reference_name, "docstatus")
					if frappe.db.exists(issue.reference_doctype, issue.reference_name)
					else None
				),
				"creation": (
					frappe.db.get_value(issue.reference_doctype, issue.reference_name, "creation")
					if frappe.db.exists(issue.reference_doctype, issue.reference_name)
					else None
				),
			}
...
@frappe.whitelist()
def resolve_health_issue(issue_name: str, action: str, reason: str | None = None):
	"""
...
def run_transaction_policy_scan():
	...
	cancelled_grace_days = cint(getattr(settings, "transaction_policy_cancelled_reference_days", 0) or 0)
	...
	cutoff_cancelled = add_days(now_datetime(), -cancelled_grace_days) if cancelled_grace_days else None
	...
	for dt_info in tx_doctypes:
		...
		if hasattr(settings, "document_types"):
			for row in settings.document_types:
				if row.document_type == dt and getattr(row, "warn_not_submitted_document", 0):
					warn_drafts = True
					break
		...
		while True:
			...
			for d in drafts:
				...
				draft_severity = (
					"High" if warn_drafts and configured_severity != "Critical" else configured_severity
				)
				create_party_issue_if_missing(
		...
		if is_child:
			has_amended_from_field = frappe.get_meta(parent_dt).has_field("amended_from")
		else:
...
def sync_transaction_health_issues():
	...
	severity_map = {d["document_type"]: d["transaction_health_severity"] for d in _get_transaction_doctypes()}
	...
	for issue in open_issues:
		...
		if is_resolved:
			...
			if issue.severity != current_severity or getattr(issue, "severity_rank", None) != current_rank:
				frappe.db.set_value(
					"Party Issue",
					issue.name,
