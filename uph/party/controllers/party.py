...
def validate_party_master_on_document_types(doc, method=None, *args, **kwargs):
	...
	for pt, p_list in _group_by_party_type(parties_to_fetch).items():
		if not p_list:
			continue
		results = frappe.get_all(pt, filters={"name": ["in", p_list]}, fields=["name", "party_master"])
		for r in results:
	...
	def set_or_validate_party_master(d, is_main=False):
		...
		if not party_master and new_party_master:
			...
			msg = _("Party Master for {0} set to {1} automatically").format(d.doctype, new_party_master)
		...
		if party_master and new_party_master and party_master != new_party_master:
			frappe.throw(
				_("Party Master mismatch for Party {0} ({1}): Expected {2}, found {3}").format(
					party, party_type, new_party_master, party_master
				)
			)
		...
		if not party_master and is_mandatory:
			frappe.throw(
				_("Party Master is mandatory for {0}, but not set and not found on Party {1} ({2})").format(
					_(doctype), party, party_type
				)
			)
...
def _ensure_party_contact_link(doc, party_type, party, party_master=None):
	...
	pm_contact = frappe.db.get_value("Party Master", party_master, "party_primary_contact")
...
def validate_party_analytic_accounting(doc, party_master):
	...
	if not frappe.db.get_single_value("Party Master Settings", "enable_party_analytic_accounting"):
		return
...
def validate_party_master_on_target_party_type(doc, method=None, *args, **kwargs):
	...
	if method == "validate":
		if party_type_rule.get("reqd") and not doc.party_master:
			if frappe.flags.in_test and not getattr(doc, "force_validate_party_master", False):
				return
		...
		if doc.party_master:
			...
			if not allowed or (allowed and rule_fieldname):
				...
				all_linked = frappe.db.get_all(doc.doctype, filters=filters, pluck="name")
				...
				if existing:
					...
					frappe.throw(
						...
						msg=_("Party Master {0} already has a Party {1} linked (we are {2})").format(
							doc.party_master, existing, doc.name
						),
					)
		...
		if settings.enforce_cross_type_uniqueness and doc.name:
			...
			for pt in get_configured_party_types():
				...
				if frappe.db.exists(pt, doc.name):
					frappe.throw(
						_("Party '{0}' already exists as a {1}. Cross-type uniqueness is enforced.").format(
							doc.name, pt
						)
					)
	...
	if method in ("on_update", "after_rename"):
		...
		old_is_default_for_party_master = old_doc.get("is_default_for_party_master") if old_doc else 0
		if (
			...
			if doc.party_master:
				...
				SmartCache.update_party_to_pm_data(doc.doctype, doc.name, new_pm=doc.party_master)

			if old_party_master:
				update_linked_party_to_party_master_count(old_party_master)
				SmartCache.update_party_to_pm_data(doc.doctype, doc.name, old_pm=old_party_master)
	...
	if method == "on_trash" and doc.party_master:
		...
		SmartCache.update_party_to_pm_data(doc.doctype, doc.name, old_pm=doc.party_master)
...
def on_change_party_master_update_transactional_document_types(
	...
	if changes:
		...
		try:
			party.add_comment("Comment", f"🎯 {content} Assigned to → {party_master or 'NULL'}")

			if party_master and frappe.db.exists("Party Master", party_master):
				doc_pm = frappe.get_doc("Party Master", party_master)
				doc_pm.add_comment("Comment", f"{party.name} Assigned and Updated: {content}")
		except Exception as e:
			# Prevent QueryDeadlockError or other comment insertion failures from breaking the voucher sync loop
			frappe.logger("uph").warning(f"Failed to add comment to {party.name} during voucher sync: {e}")
...
def get_functional_document_types(document_type=None):
	...
	if document_type:
		doclist = [
			d for d in doclist if d.document_type == document_type or d.parent_doctype == document_type
		]
	return doclist
...
def get_party_details(
	...
	if party:
		party_default_currency = frappe.db.get_value(party_type, party, "default_currency")
	...
	if not party_details.get(address_field) and pm.party_primary_address:
		...
		if not party_details.get("address_display"):
			...
			party_details["address_display"] = get_address_display(pm.party_primary_address)
	...
	if party_master and company and transaction_currency:
		...
		if not current_account:
			...
			if enforce_strict:
				account_currency = frappe.get_cached_value("Account", current_account, "account_currency")
...
def get_hierarchical_pm_account(pm_name, company, currency, enforce_strict=False, account_field="account"):
	"""
		...
		if acc_data and acc_data.get(account_field):
			...
			if acc_meta:
				if acc_meta.get("is_group"):
					...
					is_match = not enforce_strict or acc_meta.get("account_currency") == currency
					if is_match:
