...
@frappe.whitelist()
def get_unlinked_parties(limit: int = 20, offset: int = 0, role_doctype: str | None = None):
	"""
...
@frappe.whitelist()
def get_unlinked_transactions(limit: int = 20, offset: int = 0, transaction_doctype: str | None = None):
	"""
	...
	for dt in types_to_scan:
		...
		extra_where = ""
		if party_fieldname:
			extra_where = f" AND (`{party_fieldname}` IS NOT NULL AND `{party_fieldname}` != '')"
...
def get_unlinked_suggestions(role_doctype: str, role_name: str, limit: int = 5):
	...
	if name_field:
		display_name = frappe.db.get_value(role_doctype, role_name, name_field) or role_name
	...
	choice_parties = []
	for p in parties:
		norm = p.normalized_party_name or NormalizationUtils.normalize_party_name(p.party_name or "")
		if norm:
			choices.append(norm)
	...
	# Find top matches
	matches = process.extract(normalized_input, choices, scorer=fuzz.ratio, limit=cint(limit) or 5)

	suggestions = []
	for _match_text, score, _idx in matches:
		if score < 50:
			continue
...
def link_to_party_master(role_doctype: str, role_name: str, party_master: str):
	...
	if not is_valide_party_master_to_party(party_master, role_doctype):
		frappe.throw(
			_("Party Master {0} is not valid for {1} (may be a group, disabled, or missing the role)").format(
				party_master, role_doctype
			)
		)
	...
	return {
		"success": True,
		"message": _("{0} {1} linked to Party Master {2}").format(role_doctype, role_name, party_master),
	}
...
def create_party_master_from_unlinked_role(role_doctype: str, role_name: str):
	...
	if not frappe.has_permission("Party Master", "create"):
		frappe.throw(_("Insufficient permissions to create Party Master"), frappe.PermissionError)
	...
	return {
		...
		"message": _("Created Party Master {0} and linked to {1}").format(pm.name, role_name),
	}
...
def _ensure_voucher_update(party_doc, old_party_master=None, force_sync=False):
	...
	if force_sync:
		try:
			...
			frappe.logger("uph").exception("Sync voucher update failed; falling back to enqueue")
...
def _find_best_parent_group_for_role(role_doctype: str):
	...
	group = frappe.db.get_value("Party Master", {"party_type": role_doctype, "is_group": 1}, "name")
	if group:
		return group
...
@frappe.whitelist()
def get_unlinked_issues(limit: int = 20, offset: int = 0, party_master: str | None = None):
	"""
...
def resolve_unlinked_voucher(role_doctype: str, role_name: str, party_master: str):
	...
	link_to_party_master(role_doctype, role_name, party_master)
	...
	for issue in issues:
		...
		if role_fieldname:
			...
			if voucher_role == role_name:
				...
				frappe.db.set_value(dt, name, "party_master", party_master, update_modified=False)
				# Resolve the issue
				frappe.db.set_value(
					"Party Issue",
					issue.name,
