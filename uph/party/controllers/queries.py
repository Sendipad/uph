...
from frappe.query_builder.functions import Coalesce, Count, Locate
from frappe.utils import add_months, nowdate, unique
...
def party_master_link_query(doctype, txt, searchfield, start, page_len, filters=None, reference_doctype=None):
	...
	if filters and not reference_doctype:
		reference_doctype = (
			filters.get("reference_doctype") or filters.get("doctype") or filters.get("on_doctype")
		)
...
def get_party_master(doctype, txt, searchfield, start, page_len, filters):
	...
	for key, value in filters.items():
		if key == "party_type":
			query = query.left_join(PartyMasterRole).on(PartyMasterRole.parent == PartyMaster.name)
			qb_filter_and_conditions.append(
				(PartyMaster.party_type == value) | (PartyMasterRole.party_type_role == value)
			)
		elif key in [
			"is_group",
			"disabled",
...
def get_leaf_party_master_list_from_any_node(filters):
	...
	group_party_master = set(
		frappe.db.get_all("Party Master", filters={"is_group": 1, "disabled": 0}, pluck="name")
	)
...
def get_all_vouchers_documents_with_null_or_another_party_master(
	...
	if not doctypes:
		doctypes = [d for d in doctype_rules if not frappe.get_meta(d.parent_doctype).issingle]
	...
	for d in doctypes:
		...
		conditions = (
			(Voucher.party_master.isnull()) if not party_master else (Voucher.party_master == party_master)
		)
		if parties:
...
def get_unlinked_party(filters, limit=None):
	...
	for pt in party_type:
		...
		data = frappe.get_all(pt, filters={"party_master": ["is", "not set"]}, limit=limit or 20)
		for d in data:
			d["party_type"] = pt
			results.append(d)
	return results
...
def query_similar_name_or_number(party_name=None, party_number=None):
	...
	if party_name:
		...
		if exact_name:
			...
			all_parties = frappe.db.get_all("Party Master", fields=["party_name", "normalized_party_name"])
	...
	if party_number:
		exact_number = frappe.db.get_value("Party Master", {"party_number": party_number}, "name")
		if exact_number:
...
def get_party_master_dashboard_info(party_master_name):
	...
	if pm_doc.is_group:
		...
		target_pms = get_leaf_party_master_list_from_any_node({"party_master": party_master_name})
	...
	for p_type, p_names in parties_by_type.items():
		...
		for p_name in p_names:
			...
			for info in info_list:
				...
				stats["unpaid_count"] += unpaid_counts_lookup.get((p_name, info["company"]), 0)
