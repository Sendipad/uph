...
def _fetch_candidates(doc, rule):
	...
	# Fetch candidates
	if or_filters:
		return frappe.get_all(doc.doctype, filters=filters, or_filters=or_filters, fields=field_list)
	else:
...
def handle_rule_action(rule, duplicates, doc):
	...
	msg = (
		_("<b>Data Quality Alert:</b> Potential duplicates found for rule '{0}'").format(rule.rule_name)
		+ "<br>"
	)
	msg += "<ul>"
	for d in duplicates:
