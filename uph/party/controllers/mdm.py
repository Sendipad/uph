...
def validate_document_quality(doc, method):
    ...
    rules = frappe.get_all(
        "Data Quality Rule",
        filters={"document_type": doc.doctype, "trigger": trigger, "enabled": 1},
    ...
    for rule_data in rules:
        ...
        if rule.filter_condition:
            try:
                ...
                frappe.log_error(
                    f"Error evaluating filter condition for rule {rule.name}"
                )
                continue
...
def _fetch_candidates(doc, rule):
    ...
    if or_filters:
        return frappe.get_all(
            doc.doctype, filters=filters, or_filters=or_filters, fields=field_list
        )
    else:
...
def _score_candidates(doc, rule, candidates):
    ...
    for candidate in candidates:
        ...
        if total_score >= rule.threshold_score:
            duplicates.append({"docname": candidate.name, "score": total_score})

    return duplicates


def find_duplicate_issues(doc, rule):
    """
...
def handle_rule_action(rule, duplicates, doc):
    ...
    msg = (
        _(
            "<b>Data Quality Alert:</b> Potential duplicates found for rule '{0}'"
