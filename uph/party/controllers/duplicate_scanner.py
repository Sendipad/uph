...
def run_duplicate_scan(min_score: float = 80.0, block_len: int = 2, chunk_size: int = 250):
	"""
	...
	try:
		...
		frappe.log_error(
			...
			message=_("rapidfuzz is not installed. Install with: pip install rapidfuzz"),
		)
		return
	...
	for p in parties:
		if not p.normalized_party_name:
			p.normalized_party_name = NormalizationUtils.normalize_party_name(p.party_name or "")
