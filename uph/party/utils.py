...
def setup_party_master_custom_fields():
	...
	for d in doclist:
		...
		if doctype == "Payment Entry":
			depends_on = "eval:doc.payment_type=='Pay' || doc.payment_type=='Receive'"
...
def get_party_master_list(doctype, txt, searchfield, start, page_len, filters):
	...
	# === Build base query ===
	query = frappe.qb.from_(PartyMaster).select(*[getattr(PartyMaster, f) for f in fields])
	...
	if party_type := filters.get("party_type"):
		...
		query = query.where(
			(PartyMaster.party_type == party_type) | PartyMaster.name.isin(secondary_role_subquery)
		)
		...
		if pt:
			...
			query = query.where(
				(PartyMaster.party_type == pt) | PartyMaster.name.isin(secondary_role_subquery)
			)
