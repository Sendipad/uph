...
from frappe.desk.search import search_link,get_std_fields_list
...
def get_mapped_fieldnames(doctype,fields=None):
    none=frappe._dict(
        party_fieldname=None,
        isdynamic_party_type=None,
        party_type=None,
...
def get_party_type_party_field_from_doc(doc, value=False, party_type=None):
    ...
    if doc.doctype in ("Payment Entry", "Journal Entry Account") and party_type == None:
...
def setup_party_master_custom_fields():
    ...
    for d in doclist:
        ...
        insert_after = 'naming_series' if frappe.get_meta(d).has_field('naming_series') else party_field
        ...
        dt_df.update({doctype: frappe._dict(
    ...
    for dt, df in dt_df.items():
        try:
            if cf:=frappe.get_list('Custom Field',filters={'dt':dt,'fieldname':'party_master'},fields=['name']):
...
def get_party_master_list(doctype, txt, searchfield, start, page_len, filters):
    ...
    if pt:=filters.get('party_type'):
        parent_party_role=frappe.db.get_all('Party Master Role',filters={'party_type_role':pt},pluck='parent')
        ...
        parent_party_role=frappe.db.get_all('Party Master Role',filters={'party_type_role':pt},pluck='parent')
        party_type_cond=" and (party_type ={0} OR name in {1})".format(pt,set(parent_party_role))
        ...
        for key, value in filters_items:
            if key!='party_type' and key!='on_doctype':
                if meta.has_field(key):
                    and_cond.append("{key} = {value}".format(key=key,value=filters.get(key)))
