...
frappe.query_reports["Party Account Statement"] = {
	filters: [
		...
		},
		{
			...
			options: [
				{
					label: __("Hide Warnings Message of Transaction Health Issue"),
		...
		},
		{
			...
			label: __("Severity Threshold"),
			fieldtype: "Select",
			options: ["Low", "Medium", "High", "Critical"],
			default: "Medium",
			depends_on: 'eval:!in_list(doc.display_options || [], "Hide Warnings Message")',
		},
		{
			fieldtype: "Break",
		},
	...
	"formatter": function (value, row, column, data, default_formatter) {
		...
		if (data && data.warning && column.fieldname === "remarks") {
			...
			if (msg) {
				return "<span style='color:var(--text-color);'>" + msg + "</span>";
			}
		}
		if (data && data.bold) {
