...
def get_timeline_chart_by_currency(data, from_date=None):
	...
	for currency, month_map in currency_month_map.items():
		...
		for month in sorted_months:
			...
			line_values.append(cumulative_balance)
...
def get_data(filters, party_master):
	...
	if not hide_warning:
		pm_list = list({p.get("party_master") for p in partylist if p.get("party_master")})
		if pm_list:
	...
	for party in partylist:
		...
		if total_balance:
			...
			if filters.get("in_company_currency"):
				closing_row["credit_in_cc" if total_balance_in_cc < 0 else "debit_in_cc"] = abs(
					total_balance_in_cc
				)
		...
		if current_pm != last_pm and not hide_warning:
			...
			valid_issues = [iss for iss in issues_for_pm if severity_levels.get(iss.severity, 0) >= min_sev]
...
def query_gl(filters, dimension):
	...
	gl_query = frappe.qb.from_(GL).select(*fields).where((entry_condition) & (period_condition))
