...
from ruzaqi_app.ruzaqi_app.report.sub_accounts_gl_summary.sub_accounts_gl_summary import (
    get_sales_invoice_count,
    get_purchase_invoice_count,
)
...
class PartyLedger(object):
    ...
    def get_conditions(self):
        ...
        self.conditions = (
            "and {}".format(" and ".join(conditions)) if conditions else ""
        )
    ...
    def get_parties(self):
        ...
        query = frappe.db.sql(
            """
				...
				{order}		
            	""".format(
                where=where_claus, order=order_claus
            ),
            self.filters,
            as_dict=1,
        )
        if (
            not query
            and self.filters.party_parent
            and self.filters.presentation_currency
        ):
            frappe.msgprint(
                _("Party Profile {0} Not Activate Currency {1}").format(
                    self.filters.party_parent, self.filters.presentation_currency
                )
            )
        self.parties = query
            ...
            if gl := parties_data.get(party.name):
                ...
                self.data.append(
                    {
                        ...
                        "remarks": "{0} {1}".format(
                            ...
                            (
                                ""
                                if not party.get("warning_message")
                                else party.get("warning_message")
                            ),
                        ),
                        "indent": 0,
                        "bold": 1,
                ...
                if self.filters.get("show_documents_summary"):
                    ...
                    if (
                        ...
                        and self.sales_invoice.get(party.name)
                    ):
                        remark.append(
                            self.get_doctype_summary(
                                "Sales Invoice", self.sales_invoice.get(party.name)
                            )
                        )
                    if (
                        party.party_type == "Supplier"
                        and self.purchase_invoice
                        and self.purchase_invoice.get(party.name)
                    ):
                        remark.append(
                            self.get_doctype_summary(
                                "Purchase Invoice",
                                self.purchase_invoice.get(party.name),
                            )
                        )
                    if remark:
                        self.data.append(
        ...
        for party, data in parties_gl_data.items():
            if len(data) > 0:
                ...
                for d in data:
                    ...
                    d["balance"] = balance
                    d["balance_statues"] = "{}".format(
                        _("Dr") if d.get("balance") > 0 else _("Cr")
                    )
                    d["indent"] = 1
                balance_word = 0
                if balance < 0:
                ...
                balance_in_word = "{0}  {1}".format(
                    "" if balance_word == 0 else _("Dr") if balance > 0 else _("Cr"),
                    money_in_words(balance_word, currency),
                )
                parties_gl_data[party].append(
                    {
                        "op_en": "Yes",
                        "totals": "Yes",
                        "currency": currency,
                        ...
                        "indent": 1,
                        "bold": 1,
                        "remarks": _("Total Current Period"),
                    }
                )
                parties_gl_data[party].append(
                    {
                        ...
                        "ending": 1,
                        "remarks": "{0} {1}".format(
                            _("The End Balance"), balance_in_word
                        ),
                        "currency": currency,
                        "balance": balance,
                        "bold": 1,
            ...
            for d in gl_entries:
                ...
                if voucher_no not in voucher_dict.keys():
                    ...
                    if voucher_type == voucher_dict[voucher_no].get("voucher_type"):
                        credit = d.get("credit", 0) + voucher_dict[voucher_no].get(
                            "credit", 0
                        )
                        debit = d.get("debit", 0) + voucher_dict[voucher_no].get(
                            "debit", 0
                        )
                        voucher_dict[voucher_no].update(
    ...
    def get_doctype_summary(self, doctype, value):
        ...
        for k, v in value.items():
            liststatus.append(
                "{0} {1} {2} ".format(
                    _("Submitted") if k == 1 else _("Cancel") if k == 2 else _("Draft"),
                    _("Counts"),
                    v,
                )
            )
