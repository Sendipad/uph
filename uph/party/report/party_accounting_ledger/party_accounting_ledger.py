...
def query_gls(filters):
    ...
    if voucher_no_not_in:
        ...
        conditions.append(
            "gl.voucher_no NOT IN ({})".format(formatted_voucher_no_not_in)
        )
    ...
    if in_company_currency:
        gl_entries_fields.extend(
            ["gl.debit AS debit_in_cc", "gl.credit AS credit_in_cc"]
        )
    ...
    """.format(
        ", ".join(opening_fields), " AND ".join(conditions), from_date
    )
    ...
    """.format(
        ", ".join(gl_entries_fields), " AND ".join(conditions), from_date, to_date
    )
