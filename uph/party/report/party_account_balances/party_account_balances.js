...
frappe.query_reports["Party Account Balances"] = {
	filters: [
...

		{
			...
			on_change: function () {
				...
				if (from_date_filter) {
					if (view_mode === "Active Accounts" || view_mode === "Dormant Accounts") {
				...
				let threshold_filter = frappe.query_report.get_filter("debit_threshold");
				if (threshold_filter) {
	...
	formatter: function (value, row, column, data, default_formatter) {
		...
		if (column.fieldtype === "Currency") {
			let currency;
			if (
				frappe.query_report.get_filter_value("arrange_balances") !== "Horizontal" &&
