...
def execute(filters=None):
    ...
    message=_('This report is based on Transaction date (Timeline) If you want to see the transactions in the Party Master, please check the Party Master Transactions Report')
...
def get_data(filters):
    ...
    entries = (
        frappe.qb.from_(GL)
        .select(
            GL.name, GL.posting_date, GL.voucher_no, GL.voucher_type, GL.remarks,
            GL.party_type, GL.party, GL.voucher_subtype,
            ...
            GL.debit.as_("debit_in_cc"),GL.credit.as_("credit_in_cc"),
        )
        .where(
...
def get_timeline_chart_by_currency(data):
    ...
    for i, (currency, date_map) in enumerate(currency_date_map.items()):
        ...
        datasets.append({
            "name": _(currency),
            "values": values,
            "chartType": "line" if i % 2 == 0 else "bar"  # alternate or customize as needed
        })
