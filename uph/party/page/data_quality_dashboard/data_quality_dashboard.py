...
def get_duplicate_issues(
	limit: int = 50, offset: int = 0, min_score: float = 0, party_master: str | None = None
):
	"""
	...
	if duplicates:
		...
		for d in duplicates:
			...
			d.party_2 = d.reference_name if d.reference_doctype == "Party Master" else None
...
@frappe.whitelist()
def get_dashboard_stats(party_master: str | None = None):
	"""
	...
	def _open_count(issue_type):
		return count_map.get((issue_type, "Open"), 0) + count_map.get((issue_type, "Under Review"), 0)
...
def get_unlinked_voucher_issues(
	...
	for dt_info in tx_doctypes:
		...
		if reference_doctype and dt != reference_doctype and parent_dt != reference_doctype:
			continue
...
@frappe.whitelist()
def dismiss_duplicate(party_1: str, party_2: str, reason: str | None = None):
	"""
