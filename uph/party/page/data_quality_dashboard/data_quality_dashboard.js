...
class DataQualityDashboard {
            ...
            <div class="data-quality-container" style="padding: 1rem;">
                ...
                <div class="stats-row" style="display: flex; gap: 1rem; margin-bottom: 1.5rem; flex-wrap: wrap;">
                    <div class="stat-card" id="stat-total-parties" data-route="" style="flex: 1; min-width: 140px; padding: 1rem 1rem 1rem 1.25rem; background: var(--card-bg); border-radius: 8px; box-shadow: var(--shadow-sm); border-left: 4px solid var(--blue-500); cursor: default;">
                        <div class="stat-label" style="color: var(--text-muted); font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 0.25rem;">${__(
							"Total Parties"
						)}</div>
	...
	update_stats(stats) {
		...
		const health_total =
			(stats.draft_voucher_count || 0) + (stats.cancelled_unamended_count || 0);
	...
	render_duplicates(data) {
		...
		if (!data.duplicates || data.duplicates.length === 0) {
			content.html(
				`<div class="text-muted text-center" style="padding: 2rem;">${__(
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: start; flex-wrap: wrap; gap: 1rem;">
                        <div style="flex: 1; min-width: 200px;">
                            <div style="font-weight: 600;">
                                <a href="/app/party-master/${dup.party_1}" target="_blank">${
				dup.party_1_name || dup.party_1
			}</a>
                            </div>
                            <div class="text-muted small">${dup.party_1 || "-"}</div>
                        </div>
                        <div style="text-align: center; padding: 0 1rem;">
                            <div class="similarity-badge" style="background: ${this.get_score_color(
								dup.similarity_score
							)}; color: white; padding: 0.25rem 0.75rem; border-radius: 1rem; font-weight: 600;">
                                ${dup.similarity_score}%
                            </div>
                            <div class="text-muted small" style="margin-top: 0.25rem;">${__(
								"Similarity"
							)}</div>
                        </div>
                        <div style="flex: 1; min-width: 200px; text-align: right;">
                            <div style="font-weight: 600;">
                                <a href="/app/party-master/${dup.party_2}" target="_blank">${
				dup.party_2_name || dup.party_2
			}</a>
                            </div>
                    ...
                    <div style="margin-top: 1rem; display: flex; gap: 0.5rem; justify-content: flex-end; align-items: center;">
                        <a href="/app/party-issue/${
							dup.name
						}" target="_blank" class="btn btn-default btn-sm" title="${__(
				"Open Issue"
			)}">
                            <i class="fa fa-external-link"></i>
                        </a>
                        <div style="flex: 1;"></div>
                        <button class="btn btn-default btn-sm btn-dismiss" data-party1="${
							dup.party_1
						}" data-party2="${dup.party_2}">
                            ${__("Not a Duplicate")}
                        </button>
                        <button class="btn btn-primary btn-sm btn-merge" data-party1="${
							dup.party_1
						}" data-party2="${dup.party_2}">
                            ${__("Merge")}
                        </button>
	...
	show_merge_dialog(party1, party2) {
		if (!party1 || !party2) {
			frappe.msgprint(
				__("Unable to resolve the selected parties. Please refresh and try again.")
			);
			return;
		}
		if (party1 === party2) {
		...
		const d = new frappe.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					options: `<div class="alert alert-warning">
                        ${__(
							"The secondary party will be merged into the primary party and deleted. All linked documents will be updated."
						)}
                    </div>`,
				},
			],
			...
			primary_action: (values) => {
				...
				frappe.confirm(
					__(
						"Are you sure you want to merge {0} into {1}? This action cannot be undone.",
						[secondary, primary]
					),
					() => {
						d.hide();
	...
	render_unlinked(data) {
		...
		if (!data.unlinked || data.unlinked.length === 0) {
			content.html(
				`<div class="text-muted text-center" style="padding: 2rem;">${__(
					"All role records are linked to a Party Master"
				)}</div>`
			);
                ...
                <div class="unlinked-row" style="display: flex; align-items: center; padding: 0.75rem 1rem; border-bottom: 1px solid var(--border-color); background: var(--card-bg);">
                    <div style="flex: 2;">
                        <div style="font-weight: 500;">
                            <a href="/app/${frappe.router.slug(item.role_doctype)}/${
				item.role_name
			}" target="_blank">${item.display_name}</a>
                        </div>
                        <div class="text-muted small">${item.role_name}</div>
                    </div>
                    <div style="flex: 1;">
                        <span class="indicator-pill" style="font-size: 0.8rem;">${
							item.role_doctype
						}</span>
                    </div>
                    ...
                    <div style="flex: 1; text-align: right; display: flex; gap: 0.5rem; justify-content: flex-end;">
                        <a href="/app/party-issue/${
							item.issue
						}" target="_blank" class="btn btn-default btn-xs" title="${__(
				"Open Issue"
			)}">
                            <i class="fa fa-external-link"></i>
                        </a>
                        <button class="btn btn-default btn-xs btn-suggest" data-doctype="${
							item.role_doctype
						}" data-name="${item.role_name}" data-display="${item.display_name}">
                            ${__("Find & Link")}
                        </button>
                    </div>
                </div>
	...
	show_link_dialog(role_doctype, role_name, display_name) {
		...
		frappe.call({
			...
			callback: (r) => {
				...
				const max_score = suggestions.length
					? Math.max(...suggestions.map((s) => s.score))
					: 0;

				if (suggestions.length) {
					let html = `<div style="margin-bottom: 0.5rem; font-weight: 600;">${__(
						"Suggested Matches"
					)}</div>`;
					suggestions.forEach((s) => {
						html += `
                            <div class="suggestion-row" style="display: flex; align-items: center; padding: 0.5rem; border: 1px solid var(--border-color); border-radius: 6px; margin-bottom: 0.5rem; cursor: pointer;" data-pm="${
								s.party_master
							}">
                                <div style="flex: 2;">
                                    <div style="font-weight: 500;">${s.party_name}</div>
                                    <div class="text-muted small">${s.party_master} | ${
							__(s.party_type) || ""
						}</div>
                                </div>
                                <div style="flex: 1; text-align: right;">
                                    <span style="background: ${
										s.score >= 80 ? "var(--green-100)" : "var(--yellow-100)"
									}; color: ${
							s.score >= 80 ? "var(--green-700)" : "var(--yellow-700)"
					...
					// Click to select
					d.fields_dict.suggestions_html.$wrapper
						.find(".suggestion-row")
						.on("click", function () {
					...
					d.fields_dict.suggestions_html.$wrapper.html(
						`<div class="text-muted">${__(
							"No close matches found. Use the selector below."
						)}</div>`
					);
				}
	...
	open_party_master_quick_entry(role_doctype, role_name, display_name) {
		...
		const role_name_fallback =
			display_name && display_name.includes(":") ? role_name : display_name;
		...
		frappe.model.with_doctype(role_doctype, () => {
			...
			frappe.db.get_value(role_doctype, role_name, fields).then((r) => {
	...
	render_unlinked_vouchers(data) {
		...
		if (!data.unlinked || data.unlinked.length === 0) {
			content.html(
				`<div class="text-muted text-center" style="padding: 2rem;">${__(
					"No unlinked voucher issues found"
				)}</div>`
			);
			...
			const row = $(`
				...
				item.role_name
                        ...
                        <div class="text-muted small">${
							item.owner ? __("By") + " " + item.owner : item.role_name
						}</div>
                    </div>
                    <div style="flex: 1;">
                        <span class="indicator-pill" style="font-size: 0.8rem;">${__(
							item.role_doctype
						)}</span>
                    </div>
                    <div style="flex: 1;">${
	...
	show_voucher_link_dialog(voucher_doctype, voucher_name, voucher_display, role_name) {
		...
		frappe.db.get_value("Party Master Settings", null, "document_types").then(() => {
				...
				callback: (r) => {
					if (r.message) {
						...
						if (doc.customer) {
							...
							actual_role_name = doc.customer;
						} else if (doc.supplier) {
							role_doctype = "Supplier";
							actual_role_name = doc.supplier;
						} else if (doc.employee) {
							role_doctype = "Employee";
							actual_role_name = doc.employee;
						} else if (doc.party_type && doc.party) {
							role_doctype = doc.party_type;
							actual_role_name = doc.party;
						}

						if (!role_doctype || !actual_role_name) {
							frappe.msgprint(
								__(
									"Could not determine the underlying party role (Customer/Supplier) for {0}",
									[voucher_display]
								)
							);
							return;
						...
						this._render_voucher_link_dialog(
							voucher_doctype,
							voucher_name,
							voucher_display,
							role_doctype,
			...
			fields: [
				{
					...
					options: `
                        <div class="alert alert-info">
                            ${__(
								"This voucher relies on the <b>{0}</b> record: <b>{1}</b>. By linking this {0} to a Party Master, this voucher (and all others using it) will be resolved.",
	...
	render_health(data) {
		...
		if (!data.parties || data.parties.length === 0) {
			content.html(
				`<div class="text-muted text-center" style="padding: 2rem;">${__(
					"No transaction health issues found"
				)}</div>`
			);
		...
		data.parties.forEach((p) => {
			...
			const row = $(`
                <div class="health-row" style="display: flex; align-items: center; padding: 0.75rem 1rem; border-bottom: 1px solid var(--border-color); background: var(--card-bg);">
                    <div style="flex: 2;">
                        <div style="font-weight: 500;">
                            <a href="/app/party-master/${p.party_master}" target="_blank">${
				p.party_name
			}</a>
                        </div>
                        <div class="text-muted small">${p.party_number || "-"} | ${
				__(p.party_type) || ""
			}</div>
                    </div>
                    <div style="flex: 1;">
                        <span class="indicator-pill" style="font-size: 0.75rem;">${
							__(p.reference_doctype) || "-"
						}</span>
                    </div>
                    <div style="flex: 1; text-align: center;">
                        <span style="font-weight: 600; color: ${
							p.draft_count > 0 ? "var(--yellow-600)" : "var(--text-muted)"
						};">${p.draft_count}</span>
                    </div>
                    <div style="flex: 1; text-align: center;">
                        <span style="font-weight: 600; color: ${
							p.cancelled_unamended_count > 0 ? "var(--red-500)" : "var(--text-muted)"
						};">${p.cancelled_unamended_count}</span>
                    </div>
                    <div style="flex: 1; text-align: center;">
                        <span style="color: ${severity_color}; font-weight: 600; font-size: 0.85rem;">${__(
				p.severity
			)}</span>
                    </div>
                    <div style="flex: 1; text-align: right;">
                        <button class="btn btn-default btn-xs btn-detail" data-pm="${
							p.party_master
						}" data-dt="${p.reference_doctype}">
                            ${__("View Details")}
                        </button>
                    </div>
                </div>
		...
		frappe.call({
			...
			callback: (r) => {
				if (!r.message || !r.message.vouchers || !r.message.vouchers.length) {
					d.fields_dict.issues_html.$wrapper.html(
						`<div class="text-muted text-center" style="padding: 2rem;">${__(
							"No problematic vouchers found for {0}",
							[party_master]
						)}</div>`
					);
					return;
				...
				r.message.vouchers.forEach((v) => {
					...
					if (v.docstatus === 0 && v.issue_code === "draft_overdue") {
						actions_html += `
                            <button class="btn btn-primary btn-xs btn-action" data-action="submit" data-issue="${
								v.issue_name
							}" title="${__("Submit Document")}">
                                <i class="fa fa-check"></i> ${__("Submit")}
                            </button>
                            <button class="btn btn-default btn-xs btn-action" data-action="cancel" data-issue="${
								v.issue_name
							}" title="${__("Cancel Document")}">
                                <i class="fa fa-ban"></i>
                            </button>
                        `;
					} else if (v.docstatus === 1 && v.issue_code === "cancelled_referenced") {
						actions_html += `
                            <button class="btn btn-danger btn-xs btn-action" data-action="cancel" data-issue="${
								v.issue_name
							}" title="${__("Cancel Document")}">
					...
					actions_html += `
                        ...
                        <button class="btn btn-default btn-xs btn-action" data-action="dismiss" data-issue="${
							v.issue_name
						}" title="${__("Ignore Issue")}" style="margin-left: 4px;">
					...
					html += `
                        <div class="health-issue-row" style="display: flex; align-items: center; padding: 0.75rem 1rem; border-bottom: 1px solid var(--border-color);">
                            <div style="flex: 2;">
                                <div style="font-weight: 500;">
                                    <a href="/app/${frappe.router.slug(v.doctype)}/${
						v.name
					}" target="_blank">${v.doctype}: ${v.name}</a>
                                </div>
                                <div class="text-muted small">${
									v.docstatus === 0
										? "Draft"
				...
				d.fields_dict.issues_html.$wrapper.find(".btn-action").on("click", (e) => {
					...
					const resolve = (reason = null) => {
						...
						frappe.call({
							...
							callback: (res) => {
								if (res.message && res.message.success) {
									$btn.closest(".health-issue-row").fadeOut(300, function () {
										$(this).remove();
									});
									frappe.show_alert({
										message: res.message.message,
					...
					if (action === "dismiss") {
						...
						if (action === "submit")
							confirm_msg = __(
								"Are you sure you want to permanently Submit this document?"
							);
						if (action === "cancel")
							confirm_msg = __(
								"Are you sure you want to permanently Cancel this document?"
							);
	...
	render_pagination(total, tab) {
		...
		pagination.append(
			`<span style="padding: 0 1rem;">${__("Page {0} of {1}", [
				current_page,
				total_pages,
			])}</span>`
		);
		...
		pagination.find(".btn-prev").on("click", () => {
			if (tab === "duplicates") {
				...
				this.unlinked_vouchers_offset = Math.max(
					0,
			...
			fields: [
				{
					...
					description: __(
						"Only show duplicates with similarity score above this threshold (0-100)"
					),
				},
				{
					fieldname: "limit",
